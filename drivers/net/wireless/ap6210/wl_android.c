...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		...
		AP6210_ERR("%s call dhd_dev_reset:%d\n",__func__,ret);
		if (ret)
			goto err;
		sdioh_start(NULL, 1);
		if (!ret) {
...
err:
	...
	AP6210_ERR("%s call dhd_dev_reset error:%d\n",__func__,ret);
	sdioh_stop(NULL);
	dhd_customer_gpio_wlan_ctrl(WLAN_RESET_OFF);
	dhd_net_if_unlock(dev);
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	if (strnicmp(command, CMD_START, strlen(CMD_START)) == 0) {
		AP6210_ERR("%s, Received regular START command\n", __FUNCTION__);
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		AP6210_ERR("%s call command stop (%d)\n",__func__,bytes_written);
	}
...
int wl_android_init(void)
{
	...
	if (!iface_name[0]) {
		...
		AP6210_DEBUG("set wifi interface name \"%s\"\n",iface_name);
	}
	return ret;
}
