...
void ap6210_gpio_init(void)
{
	...
	{
        AP6210_DEBUG("get 32k clk out put pin = %d\n",ap6210_lpo);
	}
	...
	{
        AP6210_DEBUG("get ap6210_wl_regon pin = %d\n",ap6210_wl_regon);
	}
	...
	{
        AP6210_DEBUG("get ap6210_bt_regon pin = %d\n",ap6210_bt_regon);
	}
...
static int ap6210_gpio_wifi_get_res(void)
{
	...
	if (SCRIPT_PARSER_OK != script_parser_fetch(wifi_para, "wifi_usbc_id", &ops->usb_id, 1)) {
		AP6210_ERR("parse wifi_usbc_id in script.fex failed.\n" );
		return -1;
	}

	if (SCRIPT_PARSER_OK != script_parser_fetch(wifi_para, "wifi_mod_sel", &ops->module_sel, 1)) {
		AP6210_ERR("parse wifi_mod_sel in script.fex failed.\n" );
		return -1;
	}
...
int __init ap6210_gpio_wifi_init(void)
{
	...
	if (!ops->wifi_used){
        AP6210_DEBUG("wifi parameter is unused !!!\n");
		return 0;
    }

	ret = platform_device_register(&ap6210_gpio_wifi_dev);

	AP6210_DEBUG("platform_device_register ret=%d\n",ret);
