 ...
 *benn@cubietech.com
 */
#define WL_HOST_WAKE_DEF_GPIO 2 //default gpio pin for host wake irq is gpio 18
...
dhd_open(struct net_device *net)
{
	...
	if (ret != 0) {
        ...
        AP6210_ERR("%s: wl_control_wl_start with code %d\n", __FUNCTION__, ret);
	}
#endif

	ifidx = dhd_net2idx(dhd, net);
	AP6210_ERR("%s: ifidx %d\n", __FUNCTION__, ifidx);
	...
	if (dhdp) {
		...
		if (dhd) {
			...
			if (dhd->pub.busstate != DHD_BUS_DOWN) {
				...
				AP6210_DEBUG("%s: call stop operation\n", __FUNCTION__);
...
dhd_module_init(void)
{
    ...
    //commented by jenus 2016-07-19
    error = gpio_request(WL_HOST_WAKE_DEF_GPIO, "wl_host_wake");
	if (error) {
		AP6210_ERR("[%s] get wl_host_wake gpio failed (err=%d)\n", __FUNCTION__,error);
		wl_host_wake = -1;
		return -1;
	}
...
/*
	...
	if (!wl_host_wake)
	{
        AP6210_ERR("[%s] get ap6xxx_wl_host_wake gpio failed\n", __FUNCTION__);
		wl_host_wake = -1;
		return -1;
	}
	else
	{
        printk("[%s] get ap6xxx_wl_host_wake gpio (%8x) OK!\n", __func__,wl_host_wake);
	}
*/
	// end of commented by jenus 2016-07-19
	gpio_direction_input(wl_host_wake);
	wl_host_wake_irqno = gpio_to_irq(wl_host_wake);

	AP6210_ERR("got gpio=%d, mapped to irqno=%d\n", wl_host_wake, wl_host_wake_irqno);
...
//#if 0 && defined(BCMLXSDMMC) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
	...
	if (!chip_up) {
        ...
        AP6210_ERR("Power up wifi chip......OK !!!\n");
	}
#else
	dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);
#if defined(CONFIG_WIFI_CONTROL_FUNC)
...
dhd_dev_reset(struct net_device *dev, uint8 flag)
{
	...
	if (ret) {
		AP6210_ERR("%s: error dhd_bus_devreset: %d\n", __FUNCTION__, ret);
		return ret;
	}else{
