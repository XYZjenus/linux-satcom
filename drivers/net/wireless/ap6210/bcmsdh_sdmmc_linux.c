                              ...
                              const struct sdio_device_id *id)
{
	...
	if (func) {
		...
		if (id){
			AP6210_ERR("sdio_id_class: 0x%02x\n", id->class);
			AP6210_ERR("sdio_id_vender: 0x%04x\n", id->vendor);
			AP6210_ERR("sdio_id_device#: 0x%04x\n",id->device);
		}
		...
		if (func->num == 1) {
			...
			if(func->device == 0x4) { /* 4318 */
				...
				AP6210_ERR("NIC found, calling bcmsdh_probe...\n");
		...
		if (func->num == 2) {
			...
			AP6210_ERR("%s :wl_cfg80211_set_parent_dev\n",__func__);
	#endif
			AP6210_ERR("F2 found, calling bcmsdh_probe...\n");
			ret = bcmsdh_probe(&func->dev);
		}
	} else {
		AP6210_ERR("%s, sdio function == NULL !!!\n",__func__);
		ret = -ENODEV;
	}
...
int sdio_func_reg_notify(void* semaphore)
{
	...
	ret = sdio_register_driver(&dummy_sdmmc_driver);
	//return sdio_register_driver(&dummy_sdmmc_driver);
	printk("[%s] input semaphore and register sdio(%d)\n",__func__,ret);
	return ret;
}
...
int sdio_function_init(void)
{
	...
	if (!gInstance){
        AP6210_DEBUG("%s gInstance = NULL\n", __FUNCTION__);
		return -ENOMEM;
    }

	error = sdio_register_driver(&bcmsdh_sdmmc_driver);
	if (error && gInstance) {
        AP6210_DEBUG("%s sdio_register_driver error = %d\n", __FUNCTION__,error);
		kfree(gInstance);
		gInstance = 0;
	}
