...
/*start of definitions in remap.h (jenus 2016-07-29) */
struct regmap_range {
...
static int bq25890_i2c_read_regs(struct i2c_client *client, u8 reg, u8 buf[], unsigned len)
{
	int ret;
...
static int bq25890_i2c_set_regs(struct i2c_client *client, u8 reg, u8 const buf[], __u16 len)
{
	int ret;
			      ...
			      enum bq25890_fields field_id)
{
	...
	ret = bq25890_i2c_read_regs(bq->client,reg,data,1);
	if (ret < 0){
	    printk("%s : Can not read regs\n",__func__,);
		return ret;
	}
    var = (int) data[0];
	return val;
